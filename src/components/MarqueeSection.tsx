    ...
    <section className="py-8 border-y border-border/40 overflow-hidden bg-secondary/20 relative flex items-center">
      ...
      <div className="flex whitespace-nowrap will-change-transform animate-marquee hover:[animation-play-state:paused]">
        {[...capabilities, ...capabilities, ...capabilities].map((item, index) => (
          <div key={index} className="flex items-center gap-2.5 px-8 group cursor-default">
            <item.icon
              size={18}
              className="text-muted-foreground/60 group-hover:text-primary transition-colors duration-300"
            />
            <span className="text-base font-semibold text-muted-foreground/70 group-hover:text-foreground transition-colors duration-300 tracking-tight">
              {item.name}
            </span>
          </div>
        ))}
